...
def TotalGameView(path):
    # @path: directory of top_game_csv file
    
    #read data and add sum column at the end
    ...
    path_of_streamer = os.path.dirname(path)+'/game_streams'
    
    streams = sorted(find_csv_filenames(path_of_streamer, suffix=".csv"))
    
    # calculate sum using loop
    total = []
    for i in range(0, nb_of_game):
        game = pd.read_csv(path_of_streamer+'/'+streams[i])
